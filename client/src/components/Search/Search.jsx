...
const Search = () => {
  ...
  // שליפת תוצאות חיפוש מהשרת, כולל כותרת Authorization
  useEffect(() => {
    if (debouncedQuery.length > 0) {
      setLoading(true);
      const token = localStorage.getItem("authToken");
      fetch(`http://localhost:8080/api/clothes/search?query=${encodeURIComponent(debouncedQuery)}`, {
        headers: {
